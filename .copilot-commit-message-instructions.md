...
>   ```
>
> * If the branch name starts with a number (e.g., `244-feat-something`), extract that number and append it to the title as `#244`.
> * If the branch name does not start with a number, do not add any issue number.
> * At the end of the first line, include the number of modified files and lines **based on the staged changes** in this format:
>
>   ```
>   [file {number of staged files} - lines {number of staged lines}]
>   ```
>
>   * Example:
>
...
```example1
[Feat] new feature #244 [file 1 - lines 3]
...
```example2
[Fix] fix bug #102 [file 1 - lines 3]
- fix bug description
- fix bug description
- fix bug description
```
